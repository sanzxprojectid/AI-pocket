...
void showBootScreen() {
  ...
  for (int i = 0; i < 8; i++) {
    ...
    // Tampilkan log sebelumnya biar numpuk
    for (int j = 0; j <= i; j++) {
      display.println(bootLogs[j]);
    }

    // Efek loading bar palsu di bawah
    display.drawRect(0, 54, 128, 6, SSD1306_WHITE);
    display.fillRect(2, 56, (i + 1) * 16, 2, SSD1306_WHITE); // Bar nambah terus

    display.display();
    delay(150); // Kecepatan scroll (makin kecil makin ngebut)
  }

  delay(500); // Tahan bentar pas udah penuh
  display.clearDisplay();
}
...
void initRacing(int mode) {
  ...
  // Generate track curves (Easier/Smoother)
  for(int i=0; i<100; i++) {
    // Reduced amplitude for easier driving (was 5-15, now 2-6)
...
void updateRacing() {
  ...
  if (racing.speed > 0.5f) {
    float steerSense = 1.0f + (racing.speed / 50.0f);
...
void drawRacing() {
  ...
  for(int i=0; i<RACING_ROAD_SEGMENTS; i++) {
    int y = SCREEN_HEIGHT - (i * (SCREEN_HEIGHT - horizonY) / RACING_ROAD_SEGMENTS);
    if (y < horizonY) break;

    // Perspective math
    float z = i;

    int w = roadWidth * (1.0f - (float)i/RACING_ROAD_SEGMENTS);
    int curveShift = racing.roadCurvature * i * i;

    // Alternating colors for movement effect
    int stripe = ((int)(racing.trackPosition + i) % 2 == 0) ? 1 : 0;

    if (stripe) {
      display.drawLine(centerX - w + curveShift, y, centerX + w + curveShift, y, SSD1306_WHITE);
    } else {
      display.drawPixel(centerX - w + curveShift, y, SSD1306_WHITE);
      display.drawPixel(centerX + w + curveShift, y, SSD1306_WHITE);
    }
  }

  // Draw Scenery (Roadside objects)
  for(int i=0; i<10; i++) {
     if (racing.scenery[i].active) {
         if (racing.scenery[i].z > 0 && racing.scenery[i].z < RACING_ROAD_SEGMENTS) {
             float z = racing.scenery[i].z;
             int y = SCREEN_HEIGHT - (z * (SCREEN_HEIGHT - horizonY) / RACING_ROAD_SEGMENTS);
             int curveShift = racing.roadCurvature * z * z;

             // Place further out than the road edge
             int roadEdge = (roadWidth * (1.0f - z/RACING_ROAD_SEGMENTS));
             int ex = centerX + curveShift + (racing.scenery[i].side * (roadEdge + 20)); // Offset from road

             int size = 16 * (1.0f - z/RACING_ROAD_SEGMENTS);
             if (size > 1) {
                if (racing.scenery[i].type == 0) {
                    // Tree
                    display.drawLine(ex, y, ex, y-size, SSD1306_WHITE); // Trunk
                    display.drawTriangle(ex-size/2, y-size/2, ex+size/2, y-size/2, ex, y-size, SSD1306_WHITE); // Leaves
                } else {
                    // Light post
  ...
  for(int i=0; i<5; i++) {
     if (racing.enemies[i].active) {
         if (racing.enemies[i].z > 0 && racing.enemies[i].z < RACING_ROAD_SEGMENTS) {
             float z = racing.enemies[i].z;
             int y = SCREEN_HEIGHT - (z * (SCREEN_HEIGHT - horizonY) / RACING_ROAD_SEGMENTS);
             int curveShift = racing.roadCurvature * z * z;

             // Project X
             int ex = (centerX) + (racing.enemies[i].x * roadWidth * (1.0f - z/RACING_ROAD_SEGMENTS)) + curveShift;

             int size = 16 * (1.0f - z/RACING_ROAD_SEGMENTS);
             if (size > 1) {
                display.fillRect(ex - size/2, y - size, size, size, SSD1306_WHITE);
                // "Tail lights"
                display.drawPixel(ex - size/2 + 1, y - size/3, SSD1306_BLACK);
                display.drawPixel(ex + size/2 - 2, y - size/3, SSD1306_BLACK);
             }
         }
     }
  }
  ...
  // Draw Player Car (Improved)
  int carScreenX = SCREEN_WIDTH/2 + (racing.carX * 20);
  int shakeX = (screenShake > 0) ? random(-2, 3) : 0;

  // Main body
  display.fillRoundRect(carScreenX - 10 + shakeX, SCREEN_HEIGHT - 14, 20, 10, 2, SSD1306_BLACK);
  display.drawRoundRect(carScreenX - 10 + shakeX, SCREEN_HEIGHT - 14, 20, 10, 2, SSD1306_WHITE);

  // Cabin
  display.drawRect(carScreenX - 6 + shakeX, SCREEN_HEIGHT - 18, 12, 4, SSD1306_WHITE);

  // Spoiler
  display.drawLine(carScreenX - 12 + shakeX, SCREEN_HEIGHT - 12, carScreenX + 12 + shakeX, SCREEN_HEIGHT - 12, SSD1306_WHITE);
  display.drawLine(carScreenX - 12 + shakeX, SCREEN_HEIGHT - 12, carScreenX - 12 + shakeX, SCREEN_HEIGHT - 16, SSD1306_WHITE); // Wing tip
  display.drawLine(carScreenX + 12 + shakeX, SCREEN_HEIGHT - 12, carScreenX + 12 + shakeX, SCREEN_HEIGHT - 16, SSD1306_WHITE); // Wing tip

  // Wheels
  display.fillRect(carScreenX - 11 + shakeX, SCREEN_HEIGHT - 6, 3, 5, SSD1306_WHITE);
  display.fillRect(carScreenX + 8 + shakeX, SCREEN_HEIGHT - 6, 3, 5, SSD1306_WHITE);
...
void showSystemMenu(int x_offset) {
  ...
  for (int i = 0; i < 9; i++) {
    display.setCursor(x_offset + 10, 18 + i * 7); // Tighter spacing
